      ...
      for (const produto of produtos) {
        try {
          if (!produto.id || !produto.name) {
            console.log(`⚠️ Produto sem ID ou nome (${produto.id || 'SEM ID'}), pulando...`);
            produtosErro++;
            processados++;
            continue;
          }
          ...
          const variacoes = skus.map((sku) => {
            ...
            return {
              ...
              imagem_url: sku.images?.data?.[0]?.url ? String(sku.images.data[0].url) : '',
              imagens: sku.images?.data?.map(img => String(img.url || '')).filter(url => url) || [],
              opcoes: opcoes,
